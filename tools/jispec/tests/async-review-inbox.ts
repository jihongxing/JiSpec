...
async function main(): Promise<void> {
  console.log("=== Async Review Inbox Tests ===\n");

  const results: TestResult[] = [];

  results.push(record("builder emits ready async inbox from org graph reviewer assignments", () => {
    withFixture("async-inbox-ready-", (root) => {
      writeReadyFixture(root);
      const inbox = buildAsyncReviewInbox(root);

      assert.equal(inbox.kind, "jispec-async-review-inbox");
      assert.equal(inbox.status, "ready");
      assert.equal(inbox.sourceGraph.status, "ready");
      assert.equal(inbox.summary.reviewerCount, 2);
      assert.equal(inbox.summary.totalItems, 2);
      assert.equal(inbox.summary.pending, 2);
      assert.equal(inbox.summary.reviewersMissing, 0);
      assert.equal(inbox.summary.escalationReadyItems, 2);
      assert.deepEqual(inbox.reviewers.map((reviewer) => reviewer.reviewer), ["alice", "bob"]);
      assert.deepEqual(inbox.blockers, []);
    });
  }));

  results.push(record("builder blocks missing graph or reviewer coverage with explicit blocker codes", () => {
    withFixture("async-inbox-blocked-", (root) => {
      const inbox = buildAsyncReviewInbox(root);

      assert.equal(inbox.status, "blocked");
      assert.ok(inbox.blockers.includes("org_responsibility_graph_missing"));
      assert.ok(inbox.blockers.includes("owner_action_assignments_missing"));
      assert.ok(inbox.blockers.includes("reviewer_inbox_missing"));
      assert.ok(inbox.blockers.includes("audit_evidence_missing"));
    });
  }));

  results.push(record("writer materializes JSON and Markdown companion without realtime collaboration", () => {
    withFixture("async-inbox-write-", (root) => {
      writeReadyFixture(root);
      const result = writeAsyncReviewInbox(root);

      assert.equal(result.inboxPath, ".spec/operations/async-review-inbox.json");
      assert.equal(result.summaryPath, ".spec/operations/async-review-inbox.md");
      assert.equal(fs.existsSync(path.join(root, result.inboxPath)), true);
      assert.equal(fs.existsSync(path.join(root, result.summaryPath)), true);
      const saved = JSON.parse(fs.readFileSync(path.join(root, result.inboxPath), "utf-8")) as { boundary?: { realtimeCollaborationRequired?: boolean; sourceUploadRequired?: boolean; replacesVerify?: boolean } };
      ...
      assert.match(markdown, /Async Review Inbox/);
    });
  }));

  results.push(await recordAsync("doctor global and CLI expose async review inbox readiness and write inbox", async () => {
    const root = createRepoBackedFixture("jispec-async-inbox-doctor-");
    try {
      writeReadyFixture(root);
      ...
      const cli = runCli(["doctor", "global", "--root", root, "--write-review-inbox", "--json"]);
      assert.ok([0, 1].includes(cli.status ?? -1), `${cli.stderr}\n${cli.stdout}`);
      const payload = JSON.parse(cli.stdout) as { asyncReviewInboxWrite?: { inboxPath?: string; inbox?: { kind?: string } } };
      assert.equal(payload.asyncReviewInboxWrite?.inboxPath, ".spec/operations/async-review-inbox.json");
      assert.equal(payload.asyncReviewInboxWrite?.inbox?.kind, "jispec-async-review-inbox");
      assert.equal(fs.existsSync(path.join(root, ".spec", "operations", "async-review-inbox.json")), true);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  }));

  results.push(record("Console and North Star acceptance reference async review inbox evidence", () => {
    withFixture("async-inbox-console-", (root) => {
      writeReadyFixture(root);
      writeAsyncReviewInbox(root);

      const snapshot = collectConsoleLocalSnapshot(root);
      const object = snapshot.governance.objects.find((candidate) => candidate.id === "async_review_inbox");
      assert.ok(object);
      assert.equal(object.status, "available");
      assert.equal(object.summary.status, "ready");
      assert.equal(object.summary.reviewerCount, 2);
      ...
      const scenario = acceptance.scenarios.find((candidate) => candidate.id === "async_review_inbox");
      assert.ok(scenario);
      assert.equal(scenario.status, "passed");
      assert.equal(scenario.evidence?.asyncReviewInboxStatus, "ready");
      assert.equal(scenario.evidence?.asyncReviewReviewerCount, 2);
      assert.equal(scenario.evidence?.asyncReviewRealtimeCollaborationRequired, false);
    });
  }));

  results.push(record("phase-12 suite is registered in runtime-extended matrix", () => {
    const suite = TEST_SUITES.find((candidate) => candidate.file === "async-review-inbox.ts");
    assert.ok(suite);
    assert.equal(suite.area, "runtime-extended");
    assert.equal(suite.expectedTests, 6);
    assert.equal(suite.task, "North-Star-Score-Phase-12");
  }));

...
import { renderImplementText } from "../implement/implement-runner";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(fixtureRoot, ".jispec", "implement", "change-handoff", "execution-fork.json")));
    assert.ok(fs.existsSync(path.join(fixtureRoot, ".jispec", "kernel-runtime", "change-handoff", "kernel-log.json")));
    assert.ok(fs.existsSync(path.join(fixtureRoot, ".jispec", "kernel-runtime", "change-handoff", "state-snapshot.json")));
    assert.equal(result.metadata.executionFork?.canonicalTraceSummary, "session:direct_session -> implementation:iterative_mediation");
    assert.equal(result.handoffPacket?.executionFork?.canonicalTraceSummary, result.metadata.executionFork?.canonicalTraceSummary);
    assert.equal(result.metadata.ktmRuntime?.decision, "defer");
    assert.equal(result.metadata.ktmRuntime?.toStatus, "normalized");
    ...
    assert.match(formatHandoffPacket(result.handoffPacket), /Execution fork:/);
    assert.match(formatHandoffPacket(result.handoffPacket), /Canonical trace: execution-fork-/);
    assert.match(renderImplementText(result), /KTM runtime:/);
